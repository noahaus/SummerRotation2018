...
LINES=`cat 4e_SRR094438.convergent_gene_fpkm.txt | wc -l`
NO_HEADER=$(($LINES-1))
tail -n $NO_HEADER 4e_SRR094438.convergent_gene_fpkm.txt > no_header.bed
...
LINES=`cat 4e_SRR094438.divergent_gene_fpkm.txt | wc -l`
NO_HEADER=$(($LINES-1))
tail -n $NO_HEADER 4e_SRR094438.divergent_gene_fpkm.txt > no_header.bed
...
LINES=`cat 4e_SRR094438.parallel_gene_fpkm.txt | wc -l`
NO_HEADER=$(($LINES-1))
tail -n $NO_HEADER 4e_SRR094438.parallel_gene_fpkm.txt > no_header.bed
...
uniq "$final_output.bed" > "unique_$final_output.bed"
bedtools getfasta -fi Utricularia_gibba.4.1.fa -bed "unique_$final_output.bed" -fo "$final_output.fa"
